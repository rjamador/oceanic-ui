...
export interface ToolCallProps extends Omit<HTMLAttributes<HTMLDivElement>, 'title'> {
  /**
   * Tool identifier. Shown as the verb (underscores become spaces) when
   * `label` is omitted.
   */
  name?: string
  /** Visible verb. Falls back to `name` with underscores as spaces. */
  label?: string
...
function toVerb(name: string | undefined, label: string | undefined, fallback: string): string {
  if (label) return label
  if (name) return name.replaceAll('_', ' ')
  return fallback
}

// Only the transient states are worth announcing — a completed row in a
// transcript is static, and a burst of "done, done, done" on mount is noise.
function statusMessage(status: ToolCallStatus): string {
...
/**
 * A compact tool-invocation row — quiet chrome, expands for detail:
 * recessed target chip, glow on focus, danger tint on failure. Approval
 * decisions stay with the consumer (`actions`).
 */
const ToolCallRoot = forwardRef<HTMLDivElement, ToolCallProps>(
  (
    ...
    const verb = toVerb(name, label, 'Tool')
    ...
    if (status === 'pending') {
      return (
        ...
        >
          ...
          <Text as="span" variant="labelSmall" color="muted" className="shrink-0" role="status">
            {pendingLabel}
          </Text>
          {actions}
        </div>
    ...
    return (
      <div
        ...
        data-slot="tool-call"
        data-status={status}
        className={cn('aero-tool-call', status === 'error' && 'aero-tool-call-error', className)}
        {...rest}
      >
        {clickable ? (
          <button
          ...
          <div className="aero-tool-call-row aero-tool-call-row-static">{rowInner}</div>
        )}
        {expanded && hasBody ? (
          <div
            id={detailId}
            data-slot="tool-call-detail"
            className="mt-0.5 mb-1 ml-2 border-l border-[var(--control-secondary-border)] py-0.5 pr-2 pl-3.5"
          >
            {typeof children === 'string' ? (
              <pre className="m-0 max-h-48 overflow-auto [font-family:var(--font-mono)] text-[length:var(--text-xs)] leading-relaxed whitespace-pre-wrap [overflow-wrap:anywhere] text-[var(--text-muted)]">
                {children}
              </pre>
            ) : (
              children
...
const ToolCallGroup = forwardRef<HTMLDivElement, ToolCallGroupProps>(
  (
    ...
    // The +N / -N / "N failed" spans are visible next to the label, but an
    // accessible name overrides them — fold the counts in so a screen-reader
    // user gets the same summary.
    const summaryBits = [
      added > 0 && `${added} added`,
      removed > 0 && `${removed} removed`,
      errors > 0 && `${errors} failed`,
    ].filter(Boolean)
    const groupLabel = summaryBits.length ? `${label}, ${summaryBits.join(', ')}` : label
    ...
    return (
      ...
      >
        ...
        >
          ...
          {removed > 0 ? (
            <span className="shrink-0 [font-family:var(--font-mono)] text-[length:var(--text-xs)] text-[var(--danger-text)]">
              -{removed}
            </span>
          ) : null}
          {errors > 0 ? (
            <span className="shrink-0 [font-family:var(--font-body)] text-[length:var(--text-xs)] font-medium text-[var(--danger-text)]">
              {errors} failed
            </span>
          ) : null}
        </button>
